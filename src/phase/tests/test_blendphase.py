...
def test02_eval_all(variant_scalar_rgb):
    ...
    from mitsuba.render import PhaseFunctionContext, MediumInteraction3f, TransportMode
    from mitsuba.core.xml import load_dict
    ...
    expected = (1.0 - weight) * InvFourPi + weight * InvFourPi * (1.0 - g) / (
        1.0 + g
    ) ** 2
...
def test03_sample_all(variant_scalar_rgb):
    ...
    expected_b = InvFourPi * (1 - g) / (1 + g) ** 2
...
def test04_eval_components(variant_scalar_rgb):
    ...
    expected0 = (1 - weight) * InvFourPi
    assert ek.allclose(value0, expected0)

    ctx.component = 1
    value1 = phase.eval(ctx, mi, wo)
    expected1 = weight * InvFourPi * (1.0 - g) / (1.0 + g) ** 2
...
def test05_sample_components(variant_scalar_rgb):
    ...
    expected_a = (1 - weight) * InvFourPi
    wo_a, pdf_a = phase.sample(ctx, mi, 0.3, [0.5, 0.5])
    assert ek.allclose(pdf_a, expected_a)

    expected_b = (1 - weight) * InvFourPi
    ...
    expected_a = weight * InvFourPi * (1 - g) / (1 + g) ** 2
    wo_a, pdf_a = phase.sample(ctx, mi, 0.3, [0.0, 0.0])
    assert ek.allclose(pdf_a, expected_a)

    expected_b = weight * InvFourPi * (1 - g) / (1 + g) ** 2
